class UsersController < ApplicationController
  ...
  def index
    ...
    today_transactions = @transactions.where("date_trunc('day', created_at) = ?", Time.zone.today)

    @top_credited = today_transactions.order(credit: :desc).limit(5)
    @top_debited = today_transactions.where("date_trunc('day', created_at) = ?", Time.zone.today).order(debit: :desc).limit(5)
